...
ifeq ($(UNAME_S),Darwin)
	CC = clang++
	L_BOOST = -L/usr/local/lib 
	I_BOOST = -I/usr/local/include
endif
...
libgtest.a: $(GTEST_DIR)/src/gtest-all.cc
	$(CC) -std=c++11 -stdlib=libc++ ${GTEST_I} -pthread -c $^
...
config_parser_test.o: config_parser_test.cc config_parser.h
	$(CC) -std=c++11 -g $(GTEST_I) -pthread -c config_parser_test.cc
...
config_parser_test: config_parser.o config_parser_test.o libgtest.a gtest_main.o
	$(CC) -pthread $^ -o config_parser_test
